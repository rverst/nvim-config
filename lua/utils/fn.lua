...
M.clearUpdate = function() M.isUpdate(true) end
...
M.getOs = function()
  ...
  elseif sys:find('^[Ll]inux') ~= nil then
    if string.find(rel, '[Ww][Ss][Ll]') ~= nil then return OS.WSL end
    return OS.Linux
...
M.isDir = function(path) return M.exists(M.joinPath(path, package.config:sub(1, 1))) end

M.isGui = function() return vim.fn.exists(':GuiFont') > 0 end
...
M.joinPath = function(...)
  ...
  for i, a in ipairs(args) do
    ...
    if i < #args and a:sub(-1) ~= sep then path = path .. sep end
  end
  return path
end
